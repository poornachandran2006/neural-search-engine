...
def build_single_doc_prompt(query: str, chunks: list[dict]) -> list[dict]:
    """
    Assembles the message list for a single-doc Groq call.
    Context is built by concatenating chunk texts with source metadata headers.
    """
    context_parts = []
...
def build_map_prompt(query: str, chunks: list[dict], source_file: str) -> list[dict]:
    """
    Per-document prompt for the MAP step of Map-Reduce.
    Asks the LLM to extract only what's relevant from this document's chunks.
    """
    context_parts = []
    for i, chunk in enumerate(chunks, start=1):
        page = chunk.get("page_number", "?")
...
def build_reduce_prompt(
    query: str, per_doc_answers: list[dict]
) -> list[dict]:
    """
    Final merge prompt for the REDUCE step of Map-Reduce.
    Combines per-document answers into one coherent final response.
    """
    answers_text = "\n\n---\n\n".join(
